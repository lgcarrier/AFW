SET DEFINE OFF;
create or replace package body afw_13_page_lang_pkg
as
  ...
  is
    ...
    vva_progr_a_lancr        varchar2 (4000)
                               default    'begin '
                                       || '  afw_13_page_lang_pkg.gva_libl := '
                                       || kva_const_fonct_genrt
                                       || '(:1, :2); '
                                       || '  exception when others then ete(); afw_13_page_lang_pkg.gva_libl := null;'
                                       || 'end;';
  begin
    case vva_code_dv_type_expre
      ...
      then
        ...
        execute immediate vva_progr_a_lancr
          using nvl (afw_07_util_pkg.nv ('SSPI'), afw_07_util_pkg.nv ('SSPC'))
               ,pnu_lang;

        if gva_libl is not null
        then
          vva_libl   := gva_libl;
        else
