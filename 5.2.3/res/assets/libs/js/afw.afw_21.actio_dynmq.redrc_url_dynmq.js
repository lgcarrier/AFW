/**
 * @namespace afw.afw_21.actio_dynmq.redrc_url_dynmq
 */
afw.afw_21.actio_dynmq.redrc_url_dynmq = {
    insta: [],
    obten: function (pnu_id) {
        var vjs_insta;

        for (var i = 0; i < afw.afw_21.actio_dynmq.redrc_url_dynmq.insta.length; i++) {
            if (afw.afw_21.actio_dynmq.redrc_url_dynmq.insta[i].id == pnu_id) {
                vjs_insta = afw.afw_21.actio_dynmq.redrc_url_dynmq.insta[i];
                break;
            }
        }
    ...
    suprm: function (pnu_id) {
        for (var i = 0; i < afw.afw_21.actio_dynmq.redrc_url_dynmq.insta.length; i++) {
            if (afw.afw_21.actio_dynmq.redrc_url_dynmq.insta[i].id == pnu_id) {
                afw.afw_21.actio_dynmq.redrc_url_dynmq.insta.splice(i, 1);
            }
        }
    },
    /**
     * @constructor
     */
    redrc_url_dynmq: function (pjs_optio) {
