/**
 * @namespace afw.afw_21.actio_dynmq.dialg
 */
afw.afw_21.actio_dynmq.dialg = {
    insta: [],
    obten: function (pnu_id) {
        var vjs_insta;

        for (var i = 0; i < afw.afw_21.actio_dynmq.dialg.insta.length; i++) {
            if (afw.afw_21.actio_dynmq.dialg.insta[i].id == pnu_id) {
                vjs_insta = afw.afw_21.actio_dynmq.dialg.insta[i];
                break;
            }
        }
    ...
    suprm: function (pnu_id) {
        for (var i = 0; i < afw.afw_21.actio_dynmq.dialg.insta.length; i++) {
            if (afw.afw_21.actio_dynmq.dialg.insta[i].id == pnu_id) {
                afw.afw_21.actio_dynmq.dialg.insta.splice(i, 1);
    ...
    dialg: function (pjs_optio) {
        ...
        function initl(pjs_optio) {
            ...
            console.log(that.optio);
            // Insérer les valeurs SSPC et SSPI dans l'url
            if (that.optio.vva_type_url == "PAGE") {
                ...
                that.optio.vva_url = apex.item(that.optio.vva_item_contn_url).getValue();
            ...
            that.jQuery.exec_aplic_procs({
                ...
                sucs_procs: function () {
                    that.jQuery('<div id="' + that.id + '" class="afw_21_actio_dynmq_dialg"></div>').dialog({
                        autoOpen: false,
            ...
            that.jQuery("#" + that.id).bind("dialogclose_actio_dynmq", function (event, ui) {
                ...
                afw.afw_21.actio_dynmq.dialg.obten(event.target.id).fermr();
            });
        ...
        function fermr_detr() {
            afw.afw_21.actio_dynmq.dialg.suprm(that.id);
        } //END fermr_detr
