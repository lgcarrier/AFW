afw.afw_21.actio_dynmq.afw_18_depsm_tablr_form = {
    insta: [],
    obten: function (pnu_id) {
        var vjs_insta;
        for (var i = 0; i < afw.afw_21.actio_dynmq.afw_18_depsm_tablr_form.insta.length; i++) {
            if (afw.afw_21.actio_dynmq.afw_18_depsm_tablr_form.insta[i].id == pnu_id) {
                vjs_insta = afw.afw_21.actio_dynmq.afw_18_depsm_tablr_form.insta[i];
                break;
            }
        }
    ...
    suprm: function (pnu_id) {
        for (var i = 0; i < afw.afw_21.actio_dynmq.afw_18_depsm_tablr_form.insta.length; i++) {
            if (afw.afw_21.actio_dynmq.afw_18_depsm_tablr_form.insta[i].id == pnu_id) {
                afw.afw_21.actio_dynmq.afw_18_depsm_tablr_form.insta.splice(i, 1);
            }
        }
    },
    /**
     * @constructor
     */
    afw_18_depsm_tablr_form: function (pjs_optio) {
        ...
        function initl(pjs_optio) {
            ...
            if (that.jQuery("input[name='fmap'][value='" + that.optio.vva_nom_coln_cle_primr + "']").length > 0) {
                if (!that.tablr_form_cible) {
                    ...
                    addRow = function (pNewRowVals, pNewRowMap, pDispTypeMap) {
                        ...
                        afw.afw_21.actio_dynmq.afw_18_depsm_tablr_form.insta[0].initl({});
                    }
                }
        ...
        function _afect_depsm_item(pva_id_tblr_form, pva_nom_apex, pva_type_item) {
            switch (pva_type_item) {
                case "PLUGIN_COM_SKILLBUILDERS_SUPER_LOV":
                    ...
                    that.jQuery("#" + pva_nom_apex).unbind("onchange.afw_18_depsm").bind("onchange.afw_18_depsm", function () {
                ...
                case "NATIVE_TEXTAREA":
                    ...
                    that.jQuery("#" + pva_nom_apex).unbind("blur.afw_18_depsm").bind("blur.afw_18_depsm", function () {
        ...
        function _trans_range_tablr_form(pjs_page_items) {
            selct_rang = function () {
                ...
                that.jQuery("tr.afw_18_range_tablr_form.selct", that.tablr_form_cible).removeClass("selct");
            ...
            focus_item = function () {
                that.jQuery(this).closest("tr.afw_18_range_tablr_form:not(.selct)", that.tablr_form_cible).click();
            }

            //that.jQuery("tr:gt(0):not(:last-child)", that.tablr_form_cible).addClass("afw_18_range_tablr_form");
            that.jQuery("tr:gt(0)", that.tablr_form_cible).addClass("afw_18_range_tablr_form");

            that.jQuery("tr.afw_18_range_tablr_form", that.tablr_form_cible).unbind("click.afw_18_depsm", selct_rang).bind("click.afw_18_depsm", selct_rang);

            that.jQuery("tr.afw_18_range_tablr_form input, tr.afw_18_range_tablr_form select", that.tablr_form_cible).unbind('focus.afw_18_depsm', focus_item).bind('focus.afw_18_depsm', focus_item);

            if (that.jQuery("tr.afw_18_range_tablr_form label", that.tablr_form_cible).filter(function (index) {
                return that.jQuery(this).text() == that.optio.vva_nom_coln_cle_primr && that.jQuery(this).next().val() == '';
            }).length > 0) {
                //Sélectionner la nouvelle rangée
                that.jQuery("tr.afw_18_range_tablr_form label", that.tablr_form_cible).filter(function (index) {
                    return that.jQuery(this).text() == that.optio.vva_nom_coln_cle_primr && that.jQuery(this).next().val() == '';
                }).click();
            } else {
                //Sélectionner la première rangée
                that.jQuery("tr.afw_18_range_tablr_form:eq(0)", that.tablr_form_cible).click();
            }
