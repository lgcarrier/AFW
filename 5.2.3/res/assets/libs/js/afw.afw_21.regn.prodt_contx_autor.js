/**
 * @namespace afw.afw_21.regn.prodt_contx_autor
 */
afw.afw_21.regn.prodt_contx_autor = {
    insta: [],
    obten: function (pva_id_regn) {
        var vjs_insta;

        for (var i = 0; i < afw.afw_21.regn.prodt_contx_autor.insta.length; i++) {
            if (afw.afw_21.regn.prodt_contx_autor.insta[i].optio.vva_id_regn == pva_id_regn) {
                vjs_insta = afw.afw_21.regn.prodt_contx_autor.insta[i];
                break;
            }
        }
    ...
    suprm: function (pva_id_regn) {
        for (var i = 0; i < afw.afw_21.regn.prodt_contx_autor.insta.length; i++) {
            if (afw.afw_21.regn.prodt_contx_autor.insta[i].optio.vva_id_regn == pva_id_regn) {
                afw.afw_21.regn.prodt_contx_autor.insta.splice(i, 1);
            }
        }
    },
    /**
