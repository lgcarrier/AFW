/**
 * @namespace afw.afw_21.actio_dynmq.dml_obten_clob
 */
afw.afw_21.actio_dynmq.dml_obten_clob = {
    insta: [],
    obten: function (pnu_id) {
        var vjs_insta;

        for (var i = 0; i < afw.afw_21.actio_dynmq.dml_obten_clob.insta.length; i++) {
            if (afw.afw_21.actio_dynmq.dml_obten_clob.insta[i].id == pnu_id) {
                vjs_insta = afw.afw_21.actio_dynmq.dml_obten_clob.insta[i];
                break;
            }
        }
    ...
    suprm: function (pnu_id) {
        for (var i = 0; i < afw.afw_21.actio_dynmq.dml_obten_clob.insta.length; i++) {
            if (afw.afw_21.actio_dynmq.dml_obten_clob.insta[i].id == pnu_id) {
                afw.afw_21.actio_dynmq.dml_obten_clob.insta.splice(i, 1);
            }
        }
    },
    /**
     * @constructor
     */
    dml_obten_clob: function (pjs_optio) {
        ...
        function _dml_obten_clob() {
            for (var i = 0, vnu_nombr_items = that.optio.var_liste_items.length; i < vnu_nombr_items; i++) {
                //TODO::LGCARRIER
                //code obsolète...
