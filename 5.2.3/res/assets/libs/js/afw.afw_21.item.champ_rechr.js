afw.afw_21.item.champ_rechr = {
    insta: [],
    obten: function (pnu_id) {
        var vjs_insta;

        for (var i = 0; i < afw.afw_21.item.champ_rechr.insta.length; i++) {
            if (afw.afw_21.item.champ_rechr.insta[i].id == pnu_id) {
                vjs_insta = afw.afw_21.item.champ_rechr.insta[i];
                break;
            }
        }
    ...
    suprm: function (pnu_id) {
        for (var i = 0; i < afw.afw_21.item.champ_rechr.insta.length; i++) {
            if (afw.afw_21.item.champ_rechr.insta[i].id == pnu_id) {
                afw.afw_21.item.champ_rechr.insta.splice(i, 1);
            }
        }
    },
    /**
     * @constructor
     */
    champ_rechr: function (pjs_optio) {
