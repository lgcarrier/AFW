/**
 * @namespace afw.afw_21.regn.dialg
 */
afw.afw_21.regn.dialg = {
    insta: [],
    obten: function (pnu_id) {
        var vjs_insta;

        for (var i = 0; i < afw.afw_21.regn.dialg.insta.length; i++) {
            if (afw.afw_21.regn.dialg.insta[i].id == pnu_id) {
                vjs_insta = afw.afw_21.regn.dialg.insta[i];
                break;
            }
        }
    ...
    suprm: function (pnu_id) {
        for (var i = 0; i < afw.afw_21.regn.dialg.insta.length; i++) {
            if (afw.afw_21.regn.dialg.insta[i].id == pnu_id) {
                afw.afw_21.regn.dialg.insta.splice(i, 1);
            }
        }
    },
    /**
                ...
                that.jQuery("#" + that.optio.vva_id_regn)
                    .dialog({autoOpen: that.optio.vbo_ouvri_autom,
                        ...
                        close: function (event, ui) {
                            ...
                            apex.event.trigger(that.jQuery("#" + that.id), "dialogclose_regn");
                        },
