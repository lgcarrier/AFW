afw.afw_21.actio_dynmq.gestn_tablr_form = {
    initl: function () {
    },
    ...
    defnr_indic: function (type_champ,  // Type de champ input (ex: "text" ou "hidden") à traiter
                ...
                if (champ.type == type_champ && champ.value == indic_a_trait) {
                    ...
                    if (champ_action) {
                        ...
                        if (desct_champ) {
                            ...
                            champ_action.className = "afw_itf_desct";
                        }

                        // Créer l'objet "afw" associé au champ modifié
                        champ_action.afw = {};
                        champ_action.afw.nb_decml = nb_decml;
                        champ_action.afw.carct_decml = carct_decml;

                        if (typeof calcl != "undefined" && calcl !== null) {
                            champ_action.afw.x = parseInt(numr_coln, 10);
                            champ_action.afw.y = parseInt(numr_ligne, 10);
                            champ_action.afw.calcl = calcl;
                            champ_action.afw.table_champ = new Array();
                            champ_action.afw.num_champ = 0;

                            //g_champ = champ_action;
                            obten_valr_calc(champ_action.afw.calcl, champ_action, true);
                        }
                        else {
...
// OBSOLETE
function afw_itf_numrt_apex_item(numr_coln) {
    afw.afw_21.actio_dynmq.gestn_tablr_form.numrt_apex_item(numr_coln);
}

// OBSOLETE
function afw_itf_convr_ligne_titre(type_champ_titre, chain_indic_titre) {
    afw.afw_21.actio_dynmq.gestn_tablr_form.convr_ligne_titre(type_champ_titre, chain_indic_titre);
}

// OBSOLETE
function afw_itf_defnr_indic(type_champ, indic_a_trait, declg_champ, cachr_champ, desct_champ, calcl, nb_decml, carct_decml) {
    afw.afw_21.actio_dynmq.gestn_tablr_form.defnr_indic(type_champ, indic_a_trait, declg_champ, cachr_champ, desct_champ, calcl, nb_decml, carct_decml);
}

function afw_itf_pret() {
    reclc_tout();
}
...
function defnr_evenm_on_chang(champ, action) {
    ...
    if (!champ.afw.indic_on_chang) {
        champ.afw.indic_on_chang = {};
    ...
    if (action == "reclc")
        champ.afw.indic_on_chang.reclc = 1;
    else if (action == "formt")
        champ.afw.indic_on_chang.formt = 1;
}

function evenm_on_change(champ) {
    if (champ.afw && champ.afw.indic_on_chang) {
        if (champ.afw.indic_on_chang.formt == 1)
            formt_champ(champ, champ.value);
        if (champ.afw.indic_on_chang.reclc == 1)
            reclc_tout();
    }
}
...
function obten_valr_calc(obj_calcl, obj_champ, mode_init) {
    if (typeof (obj_calcl) == "object") {
        ...
        if (mode_init) {
            if (fonct == "cell") {
                ...
                return obten_valr_champ(obj_champ.afw.x + x, obj_champ.afw.y + y);
                // Stocker une référence vers l'objet correspondant à la cellule demandée
                //g_champ.afw.table_champ[g_champ.afw.num_champ] = obten_obj_champ (g_champ.afw.x + x, g_champ.afw.y + y);
                //obj_calcl[3] = g_champ.afw.num_champ++;
            }
            else {
            ...
            switch (fonct) {
                case "cell":
                    ...
                    return obten_valr_champ(obj_champ.afw.x + x, obj_champ.afw.y + y);
                    //return obten_valr_obj_champ (obj_champ.afw.table_champ[obj_calcl[3]]);
                    break;
                case "+":
                ...
                case "somme":
                    ...
                    for (var j = y1; j <= y2; j += step_y)
                        for (var i = x1; i <= x2; i += step_x)
                            total += obten_valr_champ(obj_champ.afw.x + i, obj_champ.afw.y + j);
                    return total;
                    break;
                default:
                    return 0;
...
function formt_champ(champ, valr) {
    ...
    if (champ.afw && (champ.afw.nb_decml == "auto" || champ.afw.nb_decml === ""))
        valeur_formt = "" + valr;
    else {
        var valeur_num = parseFloat(valr_texte);
        if (isNaN(valeur_num))
            valeur_num = 0;
        valeur_formt = valeur_num.toFixed(champ.afw.nb_decml);
    }
    ...
    valeur_formt = valeur_formt.replace(/[.]/, champ.afw.carct_decml);
...
function reclc_champ(champ) {
    ...
    if (typeof g_arr_reclc[champ.id] != "undefined") {
        ...
        //g_champ = champ;
        champ.afw.valeur_num = obten_valr_calc(champ.afw.calcl, champ, false);
        valeur_brute = "" + champ.afw.valeur_num;
