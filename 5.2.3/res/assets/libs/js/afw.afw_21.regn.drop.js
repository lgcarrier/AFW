/**
 * @namespace afw.afw_21.regn.drop
 */
afw.afw_21.regn.drop = {
    insta: [],
    obten: function (pva_id) {
        var vjs_insta;

        for (var i = 0; i < afw.afw_21.regn.drop.insta.length; i++) {
            if (afw.afw_21.regn.drop.insta[i].id == pva_id) {
                vjs_insta = afw.afw_21.regn.drop.insta[i];
                break;
            }
        }
    ...
    suprm: function (pva_id) {
        for (var i = 0; i < afw.afw_21.regn.drop.insta.length; i++) {
            if (afw.afw_21.regn.drop.insta[i].id == pva_id) {
                afw.afw_21.regn.drop.insta.splice(i, 1);
            }
        }
    },
    /**
