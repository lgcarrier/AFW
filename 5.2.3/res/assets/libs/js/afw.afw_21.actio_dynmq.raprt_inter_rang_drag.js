/**
 * @namespace afw.afw_21.actio_dynmq.raprt_inter_rang_drag
 */
afw.afw_21.actio_dynmq.raprt_inter_rang_drag = {
    insta: [],
    obten: function (pva_id) {
        var vjs_insta;

        for (var i = 0; i < afw.afw_21.actio_dynmq.raprt_inter_rang_drag.insta.length; i++) {
            if (afw.afw_21.actio_dynmq.raprt_inter_rang_drag.insta[i].optio.vva_id_regn == pva_id) {
                vjs_insta = afw.afw_21.actio_dynmq.raprt_inter_rang_drag.insta[i];
                break;
            }
        }
    ...
    suprm: function (pva_id) {
        for (var i = 0; i < afw.afw_21.actio_dynmq.raprt_inter_rang_drag.insta.length; i++) {
            if (afw.afw_21.actio_dynmq.raprt_inter_rang_drag.insta[i].optio.vva_id_regn == pva_id) {
                afw.afw_21.actio_dynmq.raprt_inter_rang_drag.insta.splice(i, 1);
            }
        }
    },
    /**
