/**
 * @namespace afw.afw_21.actio_dynmq.group_entet_raprt_sql
 */
afw.afw_21.actio_dynmq.group_entet_raprt_sql = {
    insta: [],
    obten: function (pva_id_regn) {
        var vjs_insta;

        for (var i = 0; i < afw.afw_21.actio_dynmq.group_entet_raprt_sql.insta.length; i++) {
            if (afw.afw_21.actio_dynmq.group_entet_raprt_sql.insta[i].optio.vva_id_regn == pva_id_regn) {
                vjs_insta = afw.afw_21.actio_dynmq.group_entet_raprt_sql.insta[i];
                break;
            }
        }
    ...
    suprm: function (pva_id_regn) {
        for (var i = 0; i < afw.afw_21.actio_dynmq.group_entet_raprt_sql.insta.length; i++) {
            if (afw.afw_21.actio_dynmq.group_entet_raprt_sql.insta[i].optio.vva_id_regn == pva_id_regn) {
                afw.afw_21.actio_dynmq.group_entet_raprt_sql.insta.splice(i, 1);
            }
        }
    },
    /**
...
afw.afw_21.actio_dynmq.group_entet_raprt_sql.trans_raprt_sql_orign = afw.afw_21.actio_dynmq.trans_raprt_sql;

afw.afw_21.actio_dynmq.trans_raprt_sql = function (pId) {
    afw.afw_21.actio_dynmq.group_entet_raprt_sql.trans_raprt_sql_orign(pId);

    // Transformer l'entête du rapport
    if (!afw.jQuery.isEmptyObject(vjs_insta = afw.afw_21.actio_dynmq.group_entet_raprt_sql.obten(pId))) {
        vjs_optio = vjs_insta.optio;
        afw.afw_21.actio_dynmq.group_entet_raprt_sql.suprm(pId);
        afw.afw_21.actio_dynmq.group_entet_raprt_sql.insta.push(new afw.afw_21.actio_dynmq.group_entet_raprt_sql.group_entet_raprt_sql(vjs_optio));
    }
};