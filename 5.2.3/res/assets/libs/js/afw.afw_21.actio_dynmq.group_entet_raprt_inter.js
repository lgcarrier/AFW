/**
 * @namespace afw.afw_21.actio_dynmq.group_entet_raprt_inter
 */
afw.afw_21.actio_dynmq.group_entet_raprt_inter = {
    insta: [],
    obten: function (pva_id_regn) {
        var vjs_insta;

        for (var i = 0; i < afw.afw_21.actio_dynmq.group_entet_raprt_inter.insta.length; i++) {
            if (afw.afw_21.actio_dynmq.group_entet_raprt_inter.insta[i].optio.vva_id_regn == pva_id_regn) {
                vjs_insta = afw.afw_21.actio_dynmq.group_entet_raprt_inter.insta[i];
                break;
            }
        }
    ...
    suprm: function (pva_id_regn) {
        for (var i = 0; i < afw.afw_21.actio_dynmq.group_entet_raprt_inter.insta.length; i++) {
            if (afw.afw_21.actio_dynmq.group_entet_raprt_inter.insta[i].optio.vva_id_regn == pva_id_regn) {
                afw.afw_21.actio_dynmq.group_entet_raprt_inter.insta.splice(i, 1);
            }
        }
    },
    /**
    ...
    group_entet_raprt_inter: function (pjs_optio, pjs_calbc) {
        ...
        function trans_raprt_inter(pjs_calbc) {
            ...
            if (that.table.length > 0) {
                ...
                for (var i = 0, nombr_range_i = that.optio.var_ligne_3.length; i < nombr_range_i; i++) {
                    if (i == 0) {
                        // Ajouter une rang�e
                ...
                for (var i = 0, nombr_range_i = that.optio.var_ligne_2.length; i < nombr_range_i; i++) {
                    if (i == 0) {
                        // Ajouter une rangée
                        that.jQuery("tr:first", that.table).before('<tr></tr>');
                    }
                ...
                //Ajouter les colonnes vides nécessaires
                if (vnu_nombr_coln_reel > vnu_nombr_coln && vnu_nombr_coln > 0) {
                    for (var i = 0, nombr_range_i = vnu_nombr_coln_reel - vnu_nombr_coln; i < nombr_range_i; i++) {
                        that.jQuery("tr:first", that.table).append('<th></th>');
                    }
                }
                vnu_nombr_coln = 0;
        ...
        function ajout_ventl() {
            ...
            afw.jQuery("tr:eq(1) th:lt(" + that.optio.vnu_nombr_coln_gauch + ")", that.table).each(function (j) {
                ...
                afw.jQuery($obj_ln).html('')
                    .addClass(that.optio.vva_clas_css_ventl)
                    .css({'verticalAlign': 'bottom', 'textAlign': 'right', 'padding': '3px'}); //Possiblement transform� ca en classe css
            });
...
afw.afw_21.actio_dynmq.group_entet_raprt_inter.trans_raprt_inter_orign = afw.afw_21.actio_dynmq.trans_raprt_inter;

afw.afw_21.actio_dynmq.trans_raprt_inter = function (pId) {
    afw.afw_21.actio_dynmq.group_entet_raprt_inter.trans_raprt_inter_orign(pId);

    // Transformer l'ent�te du rapport
    if (!afw.jQuery.isEmptyObject(vjs_insta = afw.afw_21.actio_dynmq.group_entet_raprt_inter.obten(pId))) {
        vjs_optio = vjs_insta.optio;
        afw.afw_21.actio_dynmq.group_entet_raprt_inter.suprm(pId);
        afw.afw_21.actio_dynmq.group_entet_raprt_inter.insta.push(new afw.afw_21.actio_dynmq.group_entet_raprt_inter.group_entet_raprt_inter(vjs_optio));
    }
};